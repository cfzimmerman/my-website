import styles from "../styles/components/CoursesItem.module.css";

export const ENGSCI95R = () => (
  <p className={styles.description}>
    <i>Fall 2021</i> -{" "}
    <a className={styles.textLink} href="http://tech.seas.harvard.edu/rad">
      Startup RAD
    </a>{" "}
    was a startup-for-credit opportunity for me to build Render and meet with
    about 20 other founders weekly under the mentorship of instructor{" "}
    <a
      className={styles.textLink}
      href="http://tech.seas.harvard.edu/fellows-1"
    >
      Paul Bottino
    </a>
    .
  </p>
);

export const GENED1091 = () => (
  <p className={styles.description}>
    <i>Fall 2021</i> - This class offered a guided reading of some of China’s
    most influential ancient philosophers, including Kongzi, Mengzi, Mozi,
    Laozi, Zhuangzi, and Lord Shen. My final project applied Mengzi’s philosophy
    through a landscape architecture{" "}
    <a
      className={styles.textLink}
      href="https://cory-zimmerman.notion.site/GENED-1091-Final-Project-2f40b905d9a64e89be6135da8c5f05a4"
    >
      Minecraft build
    </a>
    .
  </p>
);

export const PSY1 = () => (
  <p className={styles.description}>
    <i>Fall 2021</i> - Taught in Prof.{" "}
    <a
      className={styles.textLink}
      href="https://en.m.wikipedia.org/wiki/Daniel_Gilbert_(psychologist)"
    >
      Dan Gilbert
    </a>
    's signature style, Psych 1 was a bi-weekly series of well-polished TED
    talks. The course explored a wide range of psychological topics including
    socialization, language, anatomy, memory, and more.
  </p>
);

export const ECON1010A = () => (
  <p className={styles.description}>
    <i>Fall 2021</i> - Ec1010 with Maxim Boycko offered a conceptual and
    technical study of core microeconomic concepts and applications relating to
    consumer, firm, and market behavior.
  </p>
);

export const MATH1B = () => (
  <p className={styles.description}>
    <i>Spring 2021</i> - Math 1B focused on conceptualizing, modeling, and
    analyzing real-world solutions addressed by its target subjects. Details on
    this{" "}
    <a
      className={styles.textLink}
      href="https://www.math.harvard.edu/course/mathematics-1b-spring/"
    >
      class site
    </a>
    .
  </p>
);

export const EXPOS20 = () => (
  <p className={styles.description}>
    <i>Spring 2021</i> - Expository writing is a long-time pillar of Harvard’s
    core curriculum. This section investigated the process of writing concise,
    compelling, well-organized prose through the medium of reading and
    reflecting on George Orwell's <i>1984</i> and other published works.
  </p>
);

export const FRSEMR52E = () => (
  <p className={styles.description}>
    <i>Spring 2021</i> - Freshman Seminars connect first-year students with
    leading researchers and professors at Harvard in a close, lighthearted
    academic environment. Mine was instructed by Prof.{" "}
    <a className={styles.textLink} href="https://hongkunparklab.com">
      Hongkun Park
    </a>
    , who guided us on a high-level exploration of quantum computing, artificial
    intelligence, genome editing, and more.
  </p>
);

export const ECON10B = () => (
  <p className={styles.description}>
    <i>Spring 2021</i> - Sequel to ECON 10A (see Fall 2020) with a focus on
    macroeconomics.
  </p>
);

export const CS50 = () => (
  <p className={styles.description}>
    <i>Fall 2020</i> - CS50 was my first computer science class. Coursework
    focused primarily on building a strong CS foundation in a broad sample of
    skill areas. Check out the{" "}
    <a
      className={styles.textLink}
      href="https://cs50.harvard.edu/college/2020/fall/"
    >
      course website
    </a>{" "}
    and my class{" "}
    <a
      className={styles.textLink}
      href="https://github.com/cfzimmerman/20F-CS50"
    >
      GitHub repo
    </a>{" "}
    for more info.
  </p>
);

export const GERMAN20B = () => (
  <p className={styles.description}>
    <i>Fall 2020</i> - German 20B brought together a small group of students to
    practice speaking, writing, understanding, and altogether properly using the
    German language.
  </p>
);

export const MATH1A = () => (
  <p className={styles.description}>
    <i>Fall 2020</i> - Math 1A offered a collaborative, application-focused
    introduction to differential and integral calculus. More info available at
    this{" "}
    <a
      className={styles.textLink}
      href="https://people.math.harvard.edu/~knill/teaching/fall2020/index.html"
    >
      course page
    </a>
    .
  </p>
);

export const ECON10A = () => (
  <p className={styles.description}>
    <i>Fall 2020</i> - Ec10 was a year-long introductory course led by Profs.{" "}
    <a
      className={styles.textLink}
      href="https://en.m.wikipedia.org/wiki/David_Laibson"
    >
      David Laibson
    </a>{" "}
    and{" "}
    <a
      className={styles.textLink}
      href="https://en.m.wikipedia.org/wiki/Jason_Furman"
    >
      Jason Furman
    </a>
    . The curriculum focused on theoretical concepts and their practical
    implications. Guest speakers included{" "}
    <a
      className={styles.textLink}
      href="https://en.m.wikipedia.org/wiki/Ben_Bernanke"
    >
      Ben Bernanke
    </a>
    ,{" "}
    <a
      className={styles.textLink}
      href="https://en.m.wikipedia.org/wiki/Greg_Mankiw"
    >
      Greg Mankiw
    </a>
    ,{" "}
    <a
      className={styles.textLink}
      href="https://en.m.wikipedia.org/wiki/Carmen_Reinhart"
    >
      Carmen Reinhart
    </a>
    ,{" "}
    <a
      className={styles.textLink}
      href="https://en.m.wikipedia.org/wiki/Cecilia_Rouse"
    >
      Cecelia Rouse
    </a>
    , and more.
  </p>
);
